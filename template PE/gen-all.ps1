...
$SizeOverMax = "error"   # size vuot max: "error" = 400/2 (de 27/07) | "clamp" = ep ve max (Trial) | "" = giu nguyen

# --- Master (project bi goi, 8081) ---
$MasterService = "Department"
$MasterRename  = ""   # bang DTO cua de: ten_cot=tenFieldDTO, "" neu khong co
$MasterFilter  = "name,status"             # bang Query Parameters: param hoac param=field (TRAI=ten param URL, PHAI=field entity), "" = tu doan
$MasterUnique  = "code"                                   # field check trung theo de, "" = tu doan tu UNIQUE cua DB
$MasterStatus  = "ACTIVE,INACTIVE,CLOSED"              # CHECK constraint status, "" = ACTIVE,INACTIVE
...
$DetailService = "Employee"
$DetailRename  = ""
$DetailFilter  = "name=fullName,status"
$DetailUnique  = ""
$DetailStatus  = "ACTIVE,LEFT,RETIRED,INACTIVE"
$DetailShape   = "C"   # bang DTO cua de: B = phang + list nested rieng (Trial) | C = 1 DTO chi co object nested (PE1) | A = phang het
$DetailRules   = @(
    @{ f = 'email';    email = $true; msg = 'email is invalid' }
    @{ f = 'position'; enum = 'Manager,Developer,Staff' }
)
# Cac key rule: pattern='regex' | email=$true | enum='A,B,C' | dateMin='yyyy-MM-dd' (after, exclusive)
#               dateMax='yyyy-MM-dd' (before) | dateMaxDays=N (before today+N) | noFuture=$true | futureOnly=$true
